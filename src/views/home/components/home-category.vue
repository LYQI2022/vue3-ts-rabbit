    ...
    <ul class="menu">
      ...
      :key="item.id">
        ...
        <template v-if="item.children">
          <RouterLink :to="`/category/sub/${ item.id }`" v-for="sub in item.children?.slice(0,2)">{{ sub.name }}</RouterLink>
        </template>

        <template v-else>
          <XtxSkeleton animated :width="70" :height="20" bg="rgba(255, 255, 255, 0.2)"/>
          <XtxSkeleton style="margin-left: 5px" animated :width="40" :height="20" bg="rgba(255, 255, 255, 0.2)"/>
        </template>
      </li>
    </ul>
    <!-- 弹层 -->
